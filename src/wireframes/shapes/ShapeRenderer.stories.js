"use strict";
/*
 * mydraft.cc
 *
 * @license
 * Copyright (c) Sebastian Stehle. All rights reserved.
*/
exports.__esModule = true;
exports.Window = exports.VerticalScrollbar = exports.VerticalLine = exports.ToggleUnchecked = exports.Toggle = exports.TextInput = exports.TextArea = exports.Tabs = exports.Slider = exports.Shape = exports.Raster = exports.RadioButton = exports.Progress = exports.Phone = exports.Paragraph = exports.Numeric = exports.List = exports.Link = exports.Label = exports.Icon = exports.HorizontalScrollbar = exports.HorizontalLine = exports.Heading = exports.Grid = exports.Dropdown = exports.Comment = exports.ComboBox = exports.Checkbox = exports.Button = exports.ButtonBar = exports.Browser = void 0;
var React = require("react");
var interface_1 = require("@app/wireframes/interface");
var ShapeRenderer_1 = require("./ShapeRenderer");
var Shapes = require("./dependencies");
exports["default"] = {
    component: ShapeRenderer_1.ShapeRenderer,
    argTypes: {
        appearance: {
            table: {
                disable: true
            }
        },
        padding: {
            table: {
                disable: true
            }
        },
        plugin: {
            table: {
                disable: true
            }
        },
        renderWidth: {
            table: {
                disable: true
            }
        },
        renderHeight: {
            table: {
                disable: true
            }
        }
    }
};
var Browser = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Browser()}/>);
};
exports.Browser = Browser;
var ButtonBar = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.ButtonBar()}/>);
};
exports.ButtonBar = ButtonBar;
var Button = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Button()}/>);
};
exports.Button = Button;
var Checkbox = function () {
    return (<>
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Checkbox()} appearance={{ STATE: 'Checked' }}/>
            <hr />
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Checkbox()} appearance={{ STATE: 'Normal' }}/>
            <hr />
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Checkbox()} appearance={{ STATE: 'Interdeminate' }}/>
        </>);
};
exports.Checkbox = Checkbox;
var ComboBox = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.ComboBox()}/>);
};
exports.ComboBox = ComboBox;
var Comment = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Comment()}/>);
};
exports.Comment = Comment;
var Dropdown = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Dropdown()}/>);
};
exports.Dropdown = Dropdown;
var Grid = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Grid()}/>);
};
exports.Grid = Grid;
var Heading = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Heading()}/>);
};
exports.Heading = Heading;
var HorizontalLine = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.HorizontalLine()}/>);
};
exports.HorizontalLine = HorizontalLine;
var HorizontalScrollbar = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.HorizontalScrollbar()}/>);
};
exports.HorizontalScrollbar = HorizontalScrollbar;
var Icon = function () {
    var _a, _b;
    return (<>
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Icon()} appearance={_a = {}, _a[interface_1.DefaultAppearance.TEXT] = String.fromCharCode(0xf000), _a}/>
            <hr />
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Icon()} appearance={_b = {}, _b[interface_1.DefaultAppearance.TEXT] = String.fromCharCode(0xf001), _b}/>
        </>);
};
exports.Icon = Icon;
var Label = function () {
    var _a, _b;
    return (<>
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Label()}/>
            <hr />
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Label()} appearance={_a = {}, _a[interface_1.DefaultAppearance.TEXT] = 'Text with <', _a}/>
            <hr />
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Label()} appearance={_b = {}, _b[interface_1.DefaultAppearance.TEXT] = 'Text with >', _b}/>
        </>);
};
exports.Label = Label;
var Link = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Link()}/>);
};
exports.Link = Link;
var List = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.List()}/>);
};
exports.List = List;
var Numeric = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Numeric()}/>);
};
exports.Numeric = Numeric;
var Paragraph = function () {
    var _a, _b;
    return (<>
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Paragraph()}/>
            <hr />
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Paragraph()} appearance={_a = {}, _a[interface_1.DefaultAppearance.TEXT] = 'Lorem ipsum dolor sit amet, alii rebum postea eam ex.\nEt mei laoreet officiis, summo sensibus id me.', _a}/>
            <hr />
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Paragraph()} appearance={_b = {}, _b[interface_1.DefaultAppearance.TEXT] = 'Lorem ipsum dolor sit amet, alii rebum postea eam ex.\n\nEt mei laoreet officiis, summo sensibus id me.', _b}/>
        </>);
};
exports.Paragraph = Paragraph;
var Phone = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Phone()}/>);
};
exports.Phone = Phone;
var Progress = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Progress()}/>);
};
exports.Progress = Progress;
var RadioButton = function () {
    return (<>
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.RadioButton()} appearance={{ STATE: 'Checked' }}/>
            <hr />
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.RadioButton()} appearance={{ STATE: 'Normal' }}/>
        </>);
};
exports.RadioButton = RadioButton;
var Raster = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Raster()} appearance={{ SOURCE: IMAGE }} desiredWidth={300} desiredHeight={300}/>);
};
exports.Raster = Raster;
var Shape = function () {
    return (<>
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Shape()} appearance={{ SHAPE: 'Ellipse' }}/>
            <hr />
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Shape()} appearance={{ SHAPE: 'Rectangle' }}/>
            <hr />
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Shape()} appearance={{ SHAPE: 'Rhombus' }}/>
            <hr />
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Shape()} appearance={{ SHAPE: 'Rounded Rectangle' }}/>
            <hr />
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Shape()} appearance={{ SHAPE: 'Triangle' }}/>
        </>);
};
exports.Shape = Shape;
var Slider = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Slider()}/>);
};
exports.Slider = Slider;
var Tabs = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Tabs()}/>);
};
exports.Tabs = Tabs;
var TextArea = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.TextArea()}/>);
};
exports.TextArea = TextArea;
var TextInput = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.TextInput()}/>);
};
exports.TextInput = TextInput;
var Toggle = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Toggle()}/>);
};
exports.Toggle = Toggle;
var ToggleUnchecked = function () {
    return (<>
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Toggle()} appearance={{ STATE: 'Checked' }}/>
            <hr />
            <ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Toggle()} appearance={{ STATE: 'Normal' }}/>
        </>);
};
exports.ToggleUnchecked = ToggleUnchecked;
var VerticalLine = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.VerticalLine()}/>);
};
exports.VerticalLine = VerticalLine;
var VerticalScrollbar = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.VerticalScrollbar()}/>);
};
exports.VerticalScrollbar = VerticalScrollbar;
var Window = function () {
    return (<ShapeRenderer_1.ShapeRenderer usePreviewOffset plugin={new Shapes.Window()}/>);
};
exports.Window = Window;
var IMAGE = 'data:image/image/png;base64,';
